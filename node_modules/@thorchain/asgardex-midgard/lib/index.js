 ...
 */
const getFaultTolerance = (max) => (max >= 3 ? Math.floor(max / 3) + 1 : max);
/**
 ...
 */
const seedEndpoint = (network) => {
    const endpoint = (sub) => `https://${sub}.thorchain.info/node_ip_list.json`;
    if (network === 'chaosnet')
        return endpoint('chaosnet-seed');
    if (network === 'mainnet')
        return endpoint('seed');
    // testnet by default
    return endpoint('testnet-seed');
};
/**
 ...
 */
const midgartBaseUrl = (ip) => `http://${ip}:8080`;
/**
 ...
 * @returns Endpoint
 */
const poolAddressEndpoint = (ip) => `${midgartBaseUrl(ip)}/v1/thorchain/pool_addresses`;
/**
 ...
 * @returns Equity as boolean
 */
const comparePoolAddressLists = (a, b) => {
    if (a.length !== b.length)
        return false;
    // filter different addresses
    const differences = a.filter((pa) => !b.some((pb) => comparePoolAddress(pa, pb)));
 ...
 * @returns Sort number
 */
const sortPoolAddressByChain = (a, b) => {
    if (a.chain && b.chain) {
 ...
 * @returns `PoolAddress`
 */
const toPoolAddress = ({ address, chain }) => ({ address, chain });
/**
 ...
 */
const toPoolAddressList = (list) => list.reduce((acc, current) => [...acc, current.addresses], []);
/**
 ...
 */
const getPoolAddress = async (ip) => {
    var _a;
    const endpoint = poolAddressEndpoint(ip);
    const result = await axios$1.get(endpoint, {
        headers: getDefaultHeader()
    });
    if (!result) {
        return Promise.reject(new Error(`Could not load pool addresses from IP ${ip}`));
    }
    const endpoints = (_a = result === null || result === void 0 ? void 0 : result.data) === null || _a === void 0 ? void 0 : _a.current;
    if (!endpoints) {
        return Promise.reject(new Error(`No pool addresses available at ${ip}`));
    }
    const addresses = endpoints.map(toPoolAddress).sort(sortPoolAddressByChain);
    return { ip, addresses };
};
/**
 ...
 */
const getIpList = async (network) => {
    var _a;
    const endpoint = seedEndpoint(network);
    const response = await axios$1.get(endpoint, { headers: getDefaultHeader() });
    if (!response) {
 ...
 */
const comparePoolAddresses = (list) => {
    // get pool addresses
    ...
    if (addressesToCompare) {
        return addressList.reduce((acc, addresses) => (acc ? comparePoolAddressLists(addressesToCompare, addresses) : acc), true);
    }
    return false;
};
/**
 * Runs computation to get consensus of pool addresses
 */
const runPoolConsensus = async ({ ipList, ipListResult = [], faultTolerance, poolAddresses }) => {
    const checkList = ipList.splice(0, faultTolerance);
    // Prepare requests for pool addresses
    const poolRequests = checkList.map((ip) => getPoolAddress(ip));
    try {
        // Get all results, but ignoring failures
        const settledResult = await Promise.allSettled(poolRequests);
        // to filter out valid (not failing) results
        const poolAddressesLoaded = settledResult.reduce((acc, result) => {
            if (result.status === 'fulfilled') {
                acc.push(result.value);
            }
            return acc;
        }, []);
        // pool addresses to check = prev. + current pool addresses
        const poolAddressesToCheck = poolAddresses.concat(poolAddressesLoaded);
        // compare all pool addresses we have loaded so far
        // if all are the same (valid), store its IPs in `ipListResult`
        // if not the same, just ignore it
        const validPoolAddresses = comparePoolAddresses(poolAddressesToCheck);
        if (validPoolAddresses) {
            ipListResult = ipListResult.concat(poolAddressesLoaded.map((value) => value.ip));
        }
        // Don't we have consensus?
        if (ipListResult.length < faultTolerance) {
            // If not, try to get consensus with remaing IPs in `ipList`
            if (ipList.length) {
                // next pool addresses to check (ignore current loaded addresses if they are not valid)
                const nextPoolAddresses = validPoolAddresses ? poolAddressesToCheck : poolAddresses;
                return runPoolConsensus({ ipList, ipListResult, faultTolerance, poolAddresses: nextPoolAddresses });
            }
            // We don't have any nodes to check
            return Promise.reject(new Error(`Consensus failed - No other IPs to check`));
        }
        // We got consensus finally
        return Promise.resolve(ipListResult);
    }
    catch (e) {
 ...
 */
const getPoolConsensus = async (ipList) => {
    ...
    const faultTolerance = getFaultTolerance(ipList.length);
    // Run another computation to get consensus
    const next = (ipList) => {
        ...
        if (poolChecks < MAX_POOL_CHECKS) {
            ...
            poolChecks++;
            // Copy + shuffle list
            const list = lodash.shuffle([...ipList]);
            return getPoolConsensus(list);
        }
        else {
            // or throw an error finally
            return Promise.reject(new Error(`Unable to reach consensus for pool addresses after ${MAX_POOL_CHECKS} attempt. Try it later again.`));
        }
    };
    try {
        // Copy + shuffle list
        const list = lodash.shuffle([...ipList]);
        return await runPoolConsensus({ ipList: list, faultTolerance, poolAddresses: [] });
    }
    catch (e) {
        // whenever it failed, run it again
        return next(ipList);
    }
};
/**
 ...
 */
const cachedBaseUrl = () => {
    if (!(cachedIpList === null || cachedIpList === void 0 ? void 0 : cachedIpList.length)) {
 ...
 */
const TC_BYZANTINE_TIMESTAMP_KEY = 'tc-byzantine-ts';
const getBaseUrl = async (network, timestamp) => {
    store_legacy.set(TC_BYZANTINE_TIMESTAMP_KEY, timestamp);
    // empty chached List
    cachedIpList = [];
    // reset counter
    poolChecks = 0;
    try {
        // load ip list
        const list = await getIpList(network);
        // One node means (in most cases) start of (test|chaos|main)net and we don't need to check consensus
        if (list.length === 1) {
            cachedIpList = list;
        }
        else {
            // shuffled list to get more randomness
            const shuffledList = lodash.shuffle(list);
            // check consensus to store valid Ip's into cachedIpList
            cachedIpList = await getPoolConsensus(shuffledList);
        }
 ...
 */
const baseUrl = async (network = 'testnet', noCache = false) => {
    const now = new Date().getTime();
    const timestamp = store_legacy.get(TC_BYZANTINE_TIMESTAMP_KEY, now);
    // min. 1 hour for caching
    const minTimeDiff = 60 * 60 * 1000;
    const newCheck = noCache || now - timestamp >= minTimeDiff;
    if (newCheck || !(cachedIpList === null || cachedIpList === void 0 ? void 0 : cachedIpList.length)) {
        return getBaseUrl(network, now);
    }
    try {
